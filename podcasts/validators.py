...
def validate_naming_scheme(scheme):
    if '\\' in scheme:
        raise forms.ValidationError(_('<p>Backslashes (\\) are not allowed in scheme.'),
                                    params={'scheme': scheme})
    if scheme.startswith('/') or scheme.endswith('/'):
        raise forms.ValidationError(_('Scheme must not begin or end with \'/\''),
                                    params={'scheme': scheme})
    ...
    if len(invalid_segments) > 0:
        raise forms.ValidationError(
            ngettext(
                ...
                'segments': '\', \''.join(invalid_segments),
            }
        )
