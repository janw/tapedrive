...
class PodcastModelTestCase(TestCase):
    ...
    def test_podcast_model(self):
        ...
        self.assertIn(
            "INFO:podcasts.models.podcast:Fetched Killing Time", logs.output[0]
        )
        self.assertEqual(
            logs.output[1:],
            [
                "INFO:podcasts.models.podcast:Inserting cover image",
...
class FilenameCreationTestCase(TestCase):
    ...
    def test_valid_call(self):
        ...
        file_path = self.episode.construct_file_path(
            "", self.valid_naming_scheme, self.datefmt
        )
    ...
    def test_invalid_segment(self):
        ...
        file_path = self.episode.construct_file_path(
            "", self.invalid_naming_scheme, self.datefmt
        )
        self.assertEqual(file_path, should_be)
...
class EpisodeModelTestCase(TestCase):
    ...
    def test_episode_model(self):
        ...
        with transaction.atomic():
            episode = Episode.objects.create(guid="/testguid1/", podcast=self.podcast)
        self.assertEqual(str(episode), "Killing Time's Episode")

        with transaction.atomic():
            episode = Episode.objects.create(
                guid="/testguid2/", podcast=self.podcast, title="Fancy Test Episode"
            )
